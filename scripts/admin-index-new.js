const fs = require('fs');
const path = 'E:/blog/src/pages/admin/index.astro';
...
"      try { const res = await fetch('/api/posts'); const posts = await res.json(); renderPosts(posts); }",
"      catch (e) { list.innerHTML = '<div class=\"admin-empty\"><p>加载失败，请先运行 npm run seed 上传文章</p></div>'; }",
'    }',
'    function renderPosts(posts) {',
"      if (!posts || posts.length === 0) { list.innerHTML = '<div class=\"admin-empty\"><p>还没有文章</p><a href=\"/admin/new\" class=\"btn-new-sm\">写第一篇</a></div>'; return; }",
...
"          var target = e.currentTarget; var id = target.dataset.id; var title = target.dataset.title;",
"          if (!confirm('确定删除「' + title + '」吗？\\n此操作不可撤销。')) return;",
"          if (target.closest) target.closest('.post-card').classList.add('deleting');",
"          try { var res = await fetch('/api/posts/' + id, { method: 'DELETE' }); if (res.ok) { loadPosts(); } else { alert('删除失败'); if (target.closest) target.closest('.post-card').classList.remove('deleting'); } }",
"          catch(e) { alert('删除失败'); if (target.closest) target.closest('.post-card').classList.remove('deleting'); }",
'        });',
'      });',
'    }',
'    loadPosts();',
...
'  @media (max-width: 720px) { .admin-header { flex-direction: column; align-items: flex-start; gap: 20px; } .card-main { padding: 16px 18px; } .card-actions { flex-direction: row; width: 100%; border-left: none; border-top: 1px solid var(--line); } .card-btn { width: auto; height: 44px; } }',
'</style>',
'',
].join('\n');
...
console.log('Has post-card:', verify.includes('post-card'));
console.log('Has admin-table:', verify.includes('admin-table'));