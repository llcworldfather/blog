  ...
  <script>
    ...
    function renderPosts(posts) {
      if (!posts || posts.length === 0) {
        list.innerHTML = '<div class="admin-empty"><p>还没有文章</p></div>';
        return;
      }
      list.innerHTML = '<div class="post-cards">' + posts.map(function(p, i) {
        return '<article class="post-card" style="animation-delay:' + (i * 0.05) + 's">' +
          '<div class="card-main">' +
            '<div class="card-meta"><span class="card-date">' + new Date(p.date).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' }) + '</span><span class="card-mood">' + p.mood + '</span></div>' +
          ...
          '<div class="card-actions">' +
            ...
            '<a href="/admin/edit/' + p.id + '" class="card-btn card-btn-edit" title="edit">&#9998;</a>' +
            '<button class="card-btn card-btn-del" data-id="' + p.id + '" data-title="' + p.title + '" title="delete">&#10005;</button>' +
          '</div>' +
        '</article>';
      }).join('') + '</div>';
      list.querySelectorAll('.card-btn-del').forEach(function(btn) {
        btn.addEventListener('click', async function(e) {
          ...
          } catch(err) { alert('删除失败'); target.closest('.post-card').classList.remove('deleting'); }
        });
      });
    }
    loadPosts();
...
<style>
  ...
  .btn-new-sm { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 13px; padding: 8px 18px; background: var(--ink); color: var(--paper); border-radius: 3px; margin-top: 16px; text-decoration: none; }
  ...
  .card-btn { display: flex; align-items: center; justify-content: center; width: 48px; flex: 1; background: var(--paper-alt); color: var(--ink-soft); transition: all 0.2s ease; cursor: pointer; border: none; font-size: 16px; text-decoration: none; }
